...
function beforeSubmit(formData, jqForm, options) {
    ...
    for (i = 0, len = formData.length; i < len; ++i) {
        if (!formData[i].value && formData[i].name != 'total_tickets' && formData[i].name != 'price' &&
            formData[i].name != 'package_need_seat' && formData[i].name != 'package_need_seat_area'
            && formData[i].name != 'total_packages') {
            lackArray.push(nameMap[formData[i].name]);
        }
        else if (!formData[i].value && formData[i].name == 'total_tickets' && formData[i].name != 'package_need_seat' &&
            formData[i].name != 'package_need_seat_area' && formData[i].name != 'total_packages') {
            if (d == 0) {
                lackArray.push(nameMap[formData[i].name]);
            }
        }
        else if (!formData[i].value && formData[i].name == 'price' && formData[i].name != 'package_need_seat' &&
            formData[i].name != 'package_need_seat_area' && formData[i].name != 'total_packages') {
            if (d == 2) {
                lackArray.push(nameMap[formData[i].name]);
            }
...
function inputPackageChange(){
    var d=document.getElementById("package").value;
    if ((d == 1) || (d == 2) || (d == 3)) {
        $('#div-need_seat').hide();
        $('#div-area_arrange').hide();
        $('#div-total_tickets').hide();
