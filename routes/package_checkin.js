...
var PACAGE_ACTIVITY_DB = model.package_activities;
...
router.get("/", function(req, res, next)
{
    ...
    db[PACAGE_ACTIVITY_DB].find({_id:id4Q, status:1},function(err, docs)
    {
        ...
        var tmp = docs[0].need_package_or_not+1;
        activity = {
            name: docs[0].name,
            start_time: getTime(docs[0].start_time),
            end_time:  getTime(docs[0].end_time),
            need_package_or_not:  tmp+" 人套票",
            actid: docs[0]._id.toString()
        };
        res.render("package_activity_checkin", {activity: activity});
        return;
    });
});
...
router.post("/",function(req, res)
{
    ...
    db[PACAGE_ACTIVITY_DB].find({_id:id4Q, status:1},function(err, docs)
    {
        ...
        if (uid.length==10)
        {
            db[PACKAGE_DB].find(
                ...
                },function(err, tik)
                {
                    ...
                    {
                        db[PACKAGE_DB].update({_id:tik[0]._id},
                            {
                                $set:{status:2}
                            },function(err,result)
                            {
        ...
        {
            db[PACKAGE_DB].find({unique_id:uid},function(err, tik)
            {
                if (err || tik.length==0)
                {
                    res.json(
