...
var packact_infoer = require("../weixin_basic/package_activity_info");
...
exports.faire_bookable_activity=function(msg,res)
{
    act_infoer.getCurrentActivity_EX(function(docs)
    {
        ...
        if (docs.length===0)
        {
            packact_infoer.getCurrentActivity_EX(function(docs1)
            {
                if (docs1.length===0)
                {
                ...
                {
                    tmpEle={};
                    tmpEle[template.rich_attr.title]=docs1[i].name;
                    tmpEle[template.rich_attr.description]=docs1[i].description.replace(/\\n/g,"\n");
...
////改了
                    tmpEle[template.rich_attr.url]=urls.packactivityInfo+"?actid="+docs1[i].id;
                    tmpEle[template.rich_attr.picture]=docs1[i].pic_url;
...
//=============================================
exports.check_bookable_pack_activity=function(msg)
{
    if (msg.MsgType[0]==="text")
        if (msg.Content[0]==="套啥")
            return true;
...
exports.faire_bookable_pack_activity=function(msg,res)
{
    packact_infoer.getCurrentActivity_EX(function(docs)
    {
        if (docs.length===0)
        {
            res.send(template.getPlainTextTemplate(msg,"对不起，最近没有可以抢票的活动 orz..."));
...
////改了
            tmpEle[template.rich_attr.url]=urls.packactivityInfo+"?actid="+docs[i].id;
            tmpEle[template.rich_attr.picture]=docs[i].pic_url;
            showList.push(tmpEle);
        }
        res.send(template.getRichTextTemplate(msg,showList));
